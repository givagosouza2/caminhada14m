...
if uploaded_file is not None:
    try:
        ...
        # Padronizar nomes das colunas (remover espaços extras e converter para minúsculas)
        df.columns = df.columns.str.strip().str.lower()
        
        # Mapear as colunas específicas do seu arquivo
        col_map = {
            'tempo (ms)': 'time_ms',
            'acc x (m/s²)': 'acc_x',
            'acc y (m/s²)': 'acc_y',
        ...
        # Renomear colunas para nomes padrão internos
        ...
        # Criar funções de interpolação (interpolação linear, com extrapolação se necessário nas bordas)
        ...
        # Padrão: primeiros 10 segundos ou o conjunto total se for menor
        ...
        with col3:
            st.metric("Mín. da Norma", f"{df_window['Norma'].min():.4f}")
        with col4:
        ...
        fig_window = px.line(
            ...
            x='Tempo (s)', 
            y='Norma', 
            title=f'Norma da Aceleração na Janela Selecionada ({time_window[0]:.2f}s - {time_window[1]:.2f}s)',
            labels={'Norma': 'Norma da Aceleração (m/s²)'},
            height=300
        )
